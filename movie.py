...
def movie_proc():
    ...
    prompt = f'내가 시청한 영화는 [{watch_join}]이야, 내가 시청한 장르의 영화 중 2000년 이후에 출시되고, 내가 시청하지 않고 평점이 높은 영화 5편을 추천해줘, 출력 형식은 영화 제목을 ","로 구분한 문자열로 해줘.'
    print('-> prompt:', prompt)
    
    format = '''{"res":"영화 목록"}'''

    response = tool.answer('너는 영화 추천 시스템이야.', prompt, format = format)
